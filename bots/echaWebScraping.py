...
class EchaWebScraper:
    ...
    def start(self):
        ...
        for keyword in self.key_words:
            ...
            self.save_combined_metadata(keyword)
        self.driver.quit()
    ...
    def extract_and_save_tables(self, soup: BeautifulSoup, keyword: str, name: str, date: str):
        ...
        if tables_data:
            ...
            table_file_name = os.path.join(keyword_folder, f"{date}_{name}.json")
    ...
    def save_summary(self, keyword: str, url: str, date: str, name: str, description: str):
        ...
        os.makedirs(keyword_folder, exist_ok=True)
        summary_file_name = os.path.join(keyword_folder, f"{date}_{name}.txt")
    ...
    def save_pdf_data(self, keyword: str, data: List[dict]):
        ...
        for item in data:
            pdf_name = os.path.join(keyword_folder, f"{item['date']}_{item['file_name']}")
    ...
    def save_metadata(self, keyword: str, metadata: dict):
        ...
        metadata_file_name = os.path.join(metadata_folder,
                                          f"{metadata['notified_date']}_{metadata['name']}.json")
    ...
    def save_combined_metadata(self, keyword: str):
        """
        Combines and saves all metadata for the keyword.
        Args:
            keyword (str): Keyword for creating folder structure.
        """
        self.logger.info(f"Combining and saving metadata for keyword: {keyword}")
        keyword_folder = os.path.join('data/raw/ECHA', keyword.replace(':', '').replace(' ', '_'))
        os.makedirs(keyword_folder, exist_ok=True)
        metadata_folder = os.path.join(keyword_folder, 'metadata')
        ...
        all_metadata_file_name = os.path.join(metadata_folder, 'all_metadata.json')
        with open(all_metadata_file_name, 'w', encoding='utf-8') as f:
            json.dump(all_metadata, f, ensure_ascii=False, indent=4)
        self.logger.info(f"Combined metadata saved to {all_metadata_file_name}")
